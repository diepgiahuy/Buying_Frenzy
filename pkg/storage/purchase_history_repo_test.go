...
func TestAddHistoryRecordWithError(t *testing.T) {
	...
	_, err := historyStore.AddHistory(context.Background(), history)
	require.Error(t, err)
...
func CreateRandomHistoryRecord(t *testing.T) (model.PurchaseHistory, int64) {
	...
	got, err := historyStore.AddHistory(context.Background(), history)
...
func TestDeleteHistoryById(t *testing.T) {
	...
	err := historyStore.DeleteHistoryByID(context.Background(), historyRecord.ID)
	require.NoError(t, err)
...
func DeleteHistoryRecord(t *testing.T, historyID int64, userId int64, restaurantId int64) {
	err := historyStore.DeleteHistoryByID(context.Background(), historyID)
	require.NoError(t, err)
